...
This project is a local Kubernetes playground using `kind` and `Argo CD`,
...
**Rule:** Do not modify `.platform/` unless explicitly instructed to change
platform-level infrastructure.

## Deployment Workflow

1. **Add Chart:** Place Helm chart in `apps/<app-name>/chart/`.
2. **Define Target:** Create `apps/<app-name>/values/<cluster-name>/values.yaml`
   to enable deployment to a cluster.
3. **Sync:** Argo CD syncs automatically by default. Disable with
   `autoSync: false` in `values.yaml`.
...
- **Naming:** Follow existing naming conventions (kebab-case for
  directories/files).
- **Safety:** Always verify current context before applying changes to clusters.
- **Commits:** All commits made by the coding agent must follow
  [Conventional Commits](https://www.conventionalcommits.org/).
- **Linting:** Always lint `*.md` files after editing them using
  `markdownlint-cli2`.
